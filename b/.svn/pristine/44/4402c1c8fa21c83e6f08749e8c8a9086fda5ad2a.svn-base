...
class ConfigBase implements ConfigInterface
{
    ...
    {
        $_sys = array();
        $_sys["baseurl"] = Request::getInstance()->getBaseUrl(true);
        $_sys["app_name"] = $this->appname;
        $_sys["app_path"] = APP_PATH_DIR.$this->appname;
        $_sys["site_url"] = $_sys["baseurl"];
        $_sys["static_url"] = $_sys["baseurl"].'/static/';
        ...
        if($configset)
        {
            if(!is_array($configset) && is_file($configset)) {
                ...
                foreach($configset as $key=>$_config) {
                    if(isset($this->userInit[$key])){
                        $this->userInit[$key] = array_merge($this->userInit[$key], array_filter($_config));
                    }
                }
            }
        }
